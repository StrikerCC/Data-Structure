...
public class KDTreeNode {
    ...
    public  void insert(KDPoint pIn, int currDim, int dims){
        ...
        double value = root.coords[currDim];
        int next_dim = currDim + 1;
        if(next_dim == dims)
            next_dim = 0;
        if(pIn.coords[currDim] >= value)
            if(right == null){
                this.right = new KDTreeNode(pIn);
            }else
                this.right.insert(pIn,next_dim,dims);
        else
            if(left==null){
                this.left = new KDTreeNode(pIn);
            }else
                this.left.insert(pIn,next_dim,dims);
    }
    ...
    public KDTreeNode delete(KDPoint pIn, int currDim, int dims) {
        ...
        if(root.equals(pIn)) {
            if(this.left==null && this.right ==null)
                ...
                Pair<KDTreeNode,Integer> min = this.left.minvalue(next_dim,dims,currDim);
                root = new KDPoint(min.t.root.coords);
                KDPoint next = new KDPoint(min.t.root.coords);
                right = left;
                left = null;
                this.right = this.right.delete(next,next_dim,dims);
            }
            else{
                Pair<KDTreeNode,Integer> min = this.right.minvalue(next_dim,dims,currDim);
                root = new KDPoint(min.t.root.coords);
                KDPoint next = new KDPoint(min.t.root.coords);
                this.right = this.right.delete(next,next_dim,dims);
            }
...
;
        if(pIn.coords[currDim] >= value){
            this.right = this.right.delete(pIn,next_dim,dims);
        }
        else{
            this.left = this.left.delete(pIn,next_dim,dims);
        }
    ...
    // this is ok, finished
    // idims represent interested dims and return the reference to interested tree
    ...
    private Pair <KDTreeNode,Integer> minvalue(int currDim, int dims , int idims){

        Pair <KDTreeNode,Integer> min = new Pair(this,currDim);
        Pair <KDTreeNode,Integer> temp;
        int next_dim = currDim + 1;
        if(next_dim == dims)
            next_dim = 0;
        ...
        if(currDim == idims){
            if(this.left == null)
                ...
                return this.left.minvalue(next_dim,dims,idims);
        }
        else {
            if (this.right != null) {
                temp = this.right.minvalue(next_dim, dims, idims);
                if (temp.t.root.coords[idims] < min.t.root.coords[idims])
                    min = temp;
            }
            if (this.left != null) {
                temp = this.left.minvalue(next_dim, dims, idims);
                if (temp.t.root.coords[idims] < min.t.root.coords[idims])
                    min = temp;
            }
            return min;
       ...
       if(this.root.equals(pIn))
        ...
        if(pIn.coords[currDim] >= value)
            if(this.right == null)
                ...
                return this.right.search(pIn,next_dim,dims);

        else
            if(this.left==null)
                return false;
            else
                return this.left.search(pIn,next_dim,dims);
    }
    ...
    public void range(KDPoint anchor, Collection<KDPoint> results,
        ...
        // <= because this is inclusion querry
        if( (root.distanceSquared(anchor) <= Math.pow(range,2)) && (!root.equals(anchor)) )
            results.add(root);
        double value = root.coords[currDim];
        int next_dim = currDim + 1;
        if(next_dim == dims)
            next_dim = 0;
        double com = anchor.coords[currDim];
        // what if they are in the same line??
        if(com >= value){
            if(this.right!= null)
                this.right.range(anchor,results,range,next_dim,dims);
            if(Math.abs(value-com) <= range){
                if(this.left!=null)
                    this.left.range(anchor,results,range,next_dim,dims);
            }
        }else{
            if(this.left!= null)
                this.left.range(anchor,results,range,next_dim,dims);
    ...
    public  NNData<KDPoint> nearestNeighbor(KDPoint anchor, int currDim,
        ...
        if(root.distanceSquared(anchor) < n.bestDist && !root.equals(anchor) ){
            n.bestGuess = new KDPoint(root);
            n.bestDist = root.distanceSquared(anchor);
        }
        else if( n.bestDist == -1 && !root.equals(anchor)){
            n.bestGuess = new KDPoint(root);
            n.bestDist = root.distanceSquared(anchor);
        }

        // should we return it self or net
        double value = root.coords[currDim];
        int next_dim = currDim + 1;
        if(next_dim == dims)
            next_dim = 0;
        ...
        if(com >= value){
            if(right != null)
                this.right.nearestNeighbor(anchor,next_dim,n,dims);
            if(Math.pow(Math.abs(value - anchor.coords[currDim]),2)<=n.bestDist || n.bestDist == -1){
                if(left != null){
                    this.left.nearestNeighbor(anchor,next_dim,n,dims);
                }
            }
        }else{
            if(left != null)
    ...
    public  void kNearestNeighbors(int k, KDPoint anchor, BoundedPriorityQueue<KDPoint> queue, int currDim, int dims){
        double priority = root.distanceSquared(anchor);
        if(!root.equals(anchor)){
            queue.enqueue(new KDPoint(root),priority);
        }
        ...
        if(com >= value){
            if(right != null)
                this.right.kNearestNeighbors(k,anchor,queue,next_dim,dims);

            temp = queue.last();
            if(temp == null){
                dis = Double.MAX_VALUE;
            }else {
                dis = temp.distanceSquared(anchor);
            }

            if( Math.pow((com-value),2)<= dis || (queue.size() < k)){
                if(left != null){
                    this.left.kNearestNeighbors(k,anchor,queue,next_dim,dims);
                }
            }
        }else{
            if(left != null)
