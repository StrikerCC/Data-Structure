...
public abstract class Graph {
    ...
    public List<Integer> shortestPath(int source, int dest){
        ...
        PriorityQueue<Node_pair> Q = new PriorityQueue<Node_pair>(this.getNumNodes(),new NodeComparator());
        Set s = new HashSet();
        int num = this.getNumNodes();
        LinkedList sp = new LinkedList();
        int[] dist = new int[this.getNumNodes()];
        int[] prev= new int[this.getNumNodes()];
        /*-1 means undefined*/
        ...
        while(Q.size()!= 0){
             ...
             if(dist[u]==INFINITY)
                 break;
             int alt = 0;
             Set<Integer> h = getNeighbors(u);
             for(int v : h){
                if(s.contains(v)) {
                    alt = dist[u] + this.getEdgeWeight(u, v);
                    if (alt < dist[v]) {
                        dist[v] = alt;
                        prev[v] = u;
                        Iterator<Node_pair> i = Q.iterator();
                        while (i.hasNext()) {
                            Node_pair p = i.next(); // must be called before you can call i.remove()
                            if (p.node == v)
                                i.remove();
                        }
                        Q.add(new Node_pair(v, alt));
                    }
                }
             }
        }
        int close_dist = INFINITY;
        int close_node = -1;
        if(source == dest) {
            for (int i = 0; i < num; i++) {
                if (edgeBetween(i, source) && dist[i] != INFINITY) {
                    int w = getEdgeWeight(i, source) + dist[i];
                    if (w < close_dist) {
                        close_dist = w;
                        close_node = i;
                    }
    ...
    private class Node_pair{
        int node;
        int weight;
        Node_pair(int node, int weight){
        this.node = node;
        this.weight = weight;
        }
    }
    public class NodeComparator implements Comparator<Node_pair>{

        public int compare(Node_pair s1, Node_pair s2) {
